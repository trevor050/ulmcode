...
- `runtime_summary` writes `.ulmcode/operations/<id>/deliverables/runtime-summary.json` and `.md` for long-run handoff, including model-call split, token/cost budget rollups, per-agent usage, compaction pressure, repeated fetches, background task state, notes, and canonical artifact paths. It auto-derives model/token/cost fields from current assistant messages plus persisted child subagent sessions when those fields are omitted; explicit fields are treated as operator overrides.
