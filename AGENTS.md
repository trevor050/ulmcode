...
- During unattended active operations, `Permission.ask` and `Question.ask` use `ULMconfig.toml` `operator_timeout_seconds` when set, otherwise `goal.continuation.operatorFallbackTimeoutSeconds` (default 75s). Set `operator_timeout_seconds = 0` to wait forever. Permission timeouts reject with corrective feedback; question timeouts prefer skip/decline/unavailable choices, then recommended choices, and write artifacts under `operator-timeouts/`.
