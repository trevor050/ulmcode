...
- ULM artifact writers emit `operation.updated` after durable writes for checkpoints, plans, evidence, findings, report outlines/renders, runtime summaries, stage gates, and operation audits.
...
- `bun run --cwd packages/opencode test:ulm-lab-target` starts and probes the bundled weak-MFA, roster-IDOR, gradebook mass-assignment, storage-config leak, student-search injection, password-reset token leak, guardian invite takeover, and LTI shared-secret leak HTTP lab targets; Docker Compose support lives beside each lab under `tools/ulmcode-labs/<lab>/docker-compose.yml`.
