...
- `operation_stage_gate` checks whether a stage is ready to continue/advance and writes `deliverables/stage-gates/<stage>.json` plus `.md`. Use it at major stage boundaries; validation blocks unresolved candidate/needs-validation findings, and handoff gates can forward the same strict report quality options as `report_lint`.
