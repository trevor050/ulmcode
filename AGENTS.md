...
- `task_list` now prints `restart_args` for stale jobs when metadata is available. Prefer `task_restart` with the stale `task_id` to relaunch the same subagent session in the background; this is the recovery path after a process restart loses long-running fibers.
...
- `bun run --cwd packages/opencode test:ulm-lab-target` starts and probes the bundled weak-MFA, roster-IDOR, gradebook mass-assignment, storage-config leak, student-search injection, and password-reset token leak HTTP lab targets; Docker Compose support lives beside each lab under `tools/ulmcode-labs/<lab>/docker-compose.yml`.
