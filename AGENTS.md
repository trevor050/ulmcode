...
- `report_render` publishes print-ready HTML, an outline-aligned lightweight PDF, and a manifest to `.ulmcode/operations/<id>/deliverables/final/`. The PDF text is derived from the rendered HTML body; do not reintroduce a separate skinny PDF content path.
