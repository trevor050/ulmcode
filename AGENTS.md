...
- `operation_resume` is the first post-compaction/restart recovery tool. It emits health gaps, recommended follow-up tools, active/background tasks, and a continuation prompt before raw JSON inside `<operation_resume_json>` tags.
...
- `runtime_summary` writes `.ulmcode/operations/<id>/deliverables/runtime-summary.json` and `.md` for long-run handoff, including model-call split, token/cost budget rollups, per-agent usage, compaction pressure, repeated fetches, background task state, notes, and canonical artifact paths. It auto-derives model/token/cost fields plus compaction count/pressure from current assistant messages and persisted child subagent sessions when those fields are omitted; explicit fields are treated as operator overrides.
