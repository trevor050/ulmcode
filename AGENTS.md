...
- `runtime_summary` writes `.ulmcode/operations/<id>/deliverables/runtime-summary.json` and `.md` for long-run handoff, including model-call split, token/cost budget rollups, per-agent usage, compaction pressure, repeated fetches, background task state, notes, and canonical artifact paths. It now auto-derives model/token/cost fields from current assistant messages when those fields are omitted; explicit fields are treated as operator overrides.
