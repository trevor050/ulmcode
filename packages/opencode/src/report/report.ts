...
export namespace ReportBundle {
  ...
  export const generate = async (input: {
    ...
    const swarmQualityPath = path.join(outputDir, "swarm-quality.json")
    ...
    const swarmPolicy = await CyberEnvironment.readSwarmPolicy(session).catch(() => undefined)
    const swarmQuality = {
      swarm_aggression: swarmPolicy?.swarm_aggression ?? null,
      aggression_source: swarmPolicy ? "policy" : "default",
      derived_limits: swarmPolicy?.derived_limits ?? null,
      subagent_task_count: messages.flatMap((message) => message.parts).filter((part) => {
    ...
    await Promise.all([
      ...
      fs.writeFile(swarmQualityPath, JSON.stringify(swarmQuality, null, 2) + "\n"),
    ])
