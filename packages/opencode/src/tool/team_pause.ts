...
export const TeamPauseTool = Tool.define("team_pause", {
  description: "Pause a swarm team.",
  parameters,
  async execute(params) {
    ...
    const team = SwarmTeamManager.setStatus(params.team_id, "paused")
    if (!team) throw new Error(`Team not found: ${params.team_id}`)
    return {
      title: `Paused ${team.id}`,
      metadata: { team },
