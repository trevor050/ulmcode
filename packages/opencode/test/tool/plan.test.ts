...
describe("tool.plan", () => {
  ...
  test("plan_exit asks for swarm aggression once and persists policy", async () => {
    await using tmp = await tmpdir({
      ...
      config: {
        cyber: {
          swarm_v2_1: {
            enabled: true,
            ask_aggression_on_plan_exit: true,
          },
        },
      },
    })
  ...
  test("plan_exit does not re-ask swarm aggression when policy exists", async () => {
    await using tmp = await tmpdir({
      git: true,
      config: {
        cyber: {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        await CyberEnvironment.ensureSharedScaffold({ environment: env, session })
        await CyberEnvironment.writeSwarmPolicy({
          session: { ...session, environment: env },
          swarm_aggression: "balanced",
