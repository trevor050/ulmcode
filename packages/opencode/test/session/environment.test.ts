  ...
  test("creates scaffold for cyber sessions", async () => {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const final = await fs.stat(path.join(env.root, "deliverables", "final"))
        expect(final.isDirectory()).toBe(true)
        const archive = await fs.stat(path.join(env.root, "deliverables", "archive"))
        expect(archive.isDirectory()).toBe(true)
        const graph = await Bun.file(path.join(env.root, "agents", "coordination", "task-graph.json")).text()
        expect(graph).toContain("\"claims\"")
        const index = await Bun.file(path.join(tmp.path, "engagements", "index.json")).text()
        expect(index).toContain(session.id)
