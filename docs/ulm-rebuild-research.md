...
- The TUI now registers `ulm.operations` plus `/ulm`/`/operations`, opening a persistent auto-refreshing operation route with list, status, audit, and report readiness panels. The compact dialog still exists for modal use.
...
- Event-driven TUI operation updates. The TUI now has a pinned auto-refreshing operation route, but it still polls the typed dashboard API instead of subscribing directly to operation/background-task events.
