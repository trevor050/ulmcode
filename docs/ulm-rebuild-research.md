...
- The isolated profile also bundles the local shell non-interactive strategy and workflow commands (`btw`, `ship`, `review`, `handoff`) so fresh installs preserve prompt-safe shell behavior and common operator routines without reading the user's live `~/.config/opencode`.
