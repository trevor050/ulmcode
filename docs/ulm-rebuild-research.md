...
- `opencode ulm list`, `opencode ulm status <operationID>`, `opencode ulm resume <operationID>`, and `opencode ulm audit <operationID>` expose operation discovery, dashboards, restart briefs, and final audits directly from the CLI, with JSON output for automation.
...
- `bun run --cwd packages/opencode test:ulm-smoke` exercises a synthetic ULM lifecycle outside the unit-test helpers: operation plan, evidence, finding, report outline, final render, runtime summary, operation audit, final handoff lint, and status dashboard.
- `bun run --cwd packages/opencode test:ulm-lab` replays every `tools/ulmcode-labs/*/manifest.json` into full ULM artifacts and enforces final handoff, operation audit, and report outline-budget lint; current scenarios cover weak privileged MFA, roster cross-tenant read exposure, gradebook mass-assignment write escalation, storage/config leak, and student-search injection.
