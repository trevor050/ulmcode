...
- `operation_stage_gate` writes durable per-stage JSON/markdown gate artifacts, blocks advancement when required stage evidence is missing or validation still has unresolved candidate findings, and can forward strict report lint options for handoff gates.
