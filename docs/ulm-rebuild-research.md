...
- Full operation-wide cost/token extraction into budgets. `runtime_summary` now derives current-session, child-session, persisted background-job-session assistant usage, persisted background-job runtime snapshots, compaction counts, and background job status, and `operation_resume` flags exhausted recorded budgets; it still cannot recover cost/transcript data for older jobs that predate runtime snapshots and have no readable session ledger.
