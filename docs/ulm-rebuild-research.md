...
- `#25798` cancel subtask child sessions matters for interruption safety; covered locally by task and prompt cancellation tests that abort child sessions and finalize subtask tool state.
...
- `#25860` bare-repo worktree handling matters for autonomous worktree setup; adopted locally so project IDs still cache in the bare common dir while `project.worktree` points at the checked-out tree.
...
- `operation_resume` emits a compact post-compaction/restart brief with health gaps, recommended tools, operation-scoped tool hints, active/background tasks, and a continuation prompt before raw JSON. With `staleAfterMinutes`, it flags stale running checkpoints and stale background jobs before new work starts, recommends `operation_resume recoverStaleTasks=true` and `operation_recover` for restartable stale operation lanes, and marks exhausted runtime budgets or runtime blind-spot notes from `runtime_summary` as blocking health gaps.
...
- `operation_status` restores context after interruptions and compaction, including runtime budget/model/task rollups and runtime notes when a runtime summary exists. It now emits a compact operator dashboard before the raw JSON payload.
- `operation_stage_gate` writes durable per-stage JSON/markdown gate artifacts, blocks advancement when runtime budgets are exhausted or runtime blind spots are recorded, blocks advancement when required stage evidence is missing or validation still has unresolved candidate findings, and can forward strict report lint options for handoff gates.
...
- Restart briefs now push restartable stale background lanes toward `operation_resume` with `recoverStaleTasks: true` or `operation_recover` before duplicate replacement lanes are launched.
...
- Bare-repo-backed git worktrees now keep the project cache in the bare common dir while exposing the checked-out worktree as `project.worktree`, avoiding bare-path cwd/sidebar confusion.
...
- `report_lint` checks report readiness, outline target-page budget, required outline section coverage/density, total and per-finding report density, evidence refs, final handoff artifacts, and `finalHandoff=true`.
- `report_render` publishes final HTML, outline-aligned PDF, README, manifest, evidence index, state counts, and non-reportable finding IDs. It preserves authored `reports/report.md` or `reports/report.html` when present, then derives the PDF body from the rendered HTML body so section coverage does not drift between handoff formats.
...
- Task and prompt cancellation propagate abort signals into child subagent sessions and finalize subtask tool state instead of leaving the parent transcript stuck in a running state.
...
- `bun run --cwd packages/opencode test:ulm-lab` replays every `tools/ulmcode-labs/*/manifest.json` into full ULM artifacts and enforces validation stage gate, final handoff, operation audit, report outline-budget lint, and report outline-section lint; current scenarios cover weak privileged MFA, roster cross-tenant read exposure, gradebook mass-assignment write escalation, storage/config leak, student-search injection, password-reset token leakage, guardian invite-code takeover, LTI shared-secret launch forgery, SIS webhook signature bypass, assignment submission impersonation, attendance bulk-update CSRF, transcript export overexposure, LMS payment webhook replay, family messaging cross-class broadcast, third-party integration token leakage, a multi-finding district portal chain, and a multi-finding SSO roster/vendor sync chain.
...
- `bun run --cwd packages/opencode test:ulm-rebuild-audit` checks a prompt-to-artifact evidence checklist for upstream currency, durable operation tools, report quality gates, profile/runtime guard wiring, lab breadth, and required gate scripts. `bun run --cwd packages/opencode script/ulm-rebuild-audit.ts --json` emits the same checklist as structured JSON for automation. It complements the executable gates; it does not replace them.
- `docs/ulm-autonomy/` is the current autonomy decision hub. It maps the long-run plan to implemented model-governor, operation-engine, command-supervision, evidence/report, and harness decisions with current source links.
- `tools/ulmcode-profile/tool-manifest.json` is the current tool-acquisition and supervised-command catalog. Validate it with `bun run --cwd packages/opencode test:ulm-tool-manifest`; profile install copies it into the isolated ULM config dir.
- The harness now exposes the requested tier names: `test:ulm-harness:fast`, `test:ulm-harness:full`, `test:ulm-harness:chaos`, and `test:ulm-harness:overnight`. The overnight lane is a readiness contract for stale recovery, resume, and runtime-state behavior; real 20-hour local lab execution remains opt-in/operator-triggered.
...
- True automatic background job execution resume across process restart. Metadata is persisted, listable, stale-aware, worktree-aware, operation-recoverable with `operation_recover`, and recoverable directly from `operation_resume` when `recoverStaleTasks` is true. Restart briefs and the isolated ULM profile now push that path before duplicate lanes are launched, but in-flight fibers are not restarted without a fresh operator/model execution context.
...
- Broader vulnerable-target lab catalog expansion should remain ongoing. The catalog now covers weak-MFA, roster-IDOR, gradebook mass-assignment, storage/config leak, student-search injection, password-reset token leakage, guardian invite-code takeover, LTI shared-secret launch forgery, SIS webhook signature-bypass, assignment impersonation, attendance CSRF, transcript overexposure, LMS payment webhook replay, family messaging authorization, third-party integration token leakage, district portal chaining, and SSO-to-roster/vendor chaining with Docker Compose support. Future expansion should keep adding longer chained/report-quality cases rather than more one-finding variants.
