...
- The isolated profile ships a local `ulmcode-runtime-guard` server plugin that injects operation-resume, background-task, report-lint, runtime-summary, and final-handoff guardrails through OpenCode plugin hooks; it also vendors `@khalilgharbaoui/opencode-claude-code-plugin@0.2.2` source for audit/fork work.
...
- Full vendored third-party plugin source. The isolated profile now vendors the local OMO markdown agents/prompts/commands plus the Claude Code bridge plugin source and a local ULM runtime guard plugin, but it still does not vendor the full Oh My OpenAgent npm implementation into this repo.
