...
- `bun run --cwd packages/opencode test:ulm-lab-target` starts and probes the bundled intentionally weak MFA, roster-IDOR, gradebook mass-assignment, storage/config leak, student-search injection, password-reset token leak, guardian invite takeover, LTI shared-secret leak, and SIS webhook signature-bypass HTTP lab targets; each lab can run through its own Docker Compose file.
