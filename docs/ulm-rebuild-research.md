...
- `bun run --cwd packages/opencode test:ulm-rebuild-audit` checks a prompt-to-artifact evidence checklist for upstream currency, durable operation tools, report quality gates, profile/runtime guard wiring, lab breadth, and required gate scripts. It complements the executable gates; it does not replace them.
