...
- `report_render` publishes final HTML, outline-aligned PDF, README, manifest, evidence index, state counts, and non-reportable finding IDs. The PDF body is derived from the rendered HTML body so section coverage does not drift between handoff formats.
