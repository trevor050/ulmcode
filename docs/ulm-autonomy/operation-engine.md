...
- `ulm:literal-run-readiness <operationID> --strict --json` writes `scheduler/literal-run-readiness.json` and `.md` and separates setup readiness from literal wall-clock proof. Accelerated burn-in counts as readiness evidence only; the strict audit passes only when the daemon heartbeat and JSONL log prove the target elapsed runtime and the final daemon heartbeat records useful work such as model-lane launches, command launches, lane completions, synced jobs, or recoveries.
